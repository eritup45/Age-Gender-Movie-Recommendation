...
#     print(f'Predict rating: {pred_ans}')
    ...
    for i, ans_idx in enumerate(idx[:,0]):
        if i < 5:
            print(f"Top {i} predict rating: {pred_ans[ans_idx][0] :.3f}, movie_id: {data.iloc[ans_idx]['movie_id']}, gender: {data.iloc[ans_idx]['gender']}, age: {data.iloc[ans_idx]['age']}, user_id: {data.iloc[ans_idx]['user_id']}")
...
if __name__ == "__main__":
    print('Using model: ./save_model/xDeepFM_MSE1.0184.h5')
    print('Test data position: ./data.csv')
