...
def draw_text(img, point, text, fontScale, thickness, drawType="custom"):
    '''
...
def draw_label(img, point, label: str, drawType="custom"):
    ...
    for i, text in enumerate(label):
        if text:
            ...
            img = draw_text(img, draw_point, text, fontScale, thickness, drawType)
    return img
...
def produce_data(ages, gender):
    ...
    age = int(round(ages))
    MLs = pd.read_csv(cfg.PATH.MOVIE_LIST)
    movie_len = len(MLs)
    data = []
...
latest_top_recommendation = ''

###########################################################

@app.route('/latest_recommendation', methods=['GET'])
def get_latest_top_movie_recommendation():
    return latest_top_recommendation

@app.route('/age', methods=['GET', 'POST'])
def pred_age_gender():
    global latest_top_recommendation
    with torch.no_grad():
        """
        ...
        # TODO: pirnt
        movie_genres = request.form.getlist('genres')
        ...
        if len(detected) > 0:
            ...
            for i, (d, ages, gender) in enumerate(zip(detected, predicted_ages, predicted_gender)):
                ...
                # TODO: recommend_movies參數
                pred_movie_list, pred_movie_genres, pred_rating = recommend_movies('./recommend_system/save_model/FiBiNET_MSE1.0043.h5', test_model_input, linear_feature_columns, dnn_feature_columns, 'cuda:0', df)
