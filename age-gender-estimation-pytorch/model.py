...
#         # tresnet = torch.hub.load('rwightman/pytorch-image-models', 'tresnet_m', pretrained=True)
#         tresnet = torch.hub.load('rwightman/pytorch-image-models', 'regnetx_016', pretrained=True)
#         self.model = nn.Sequential(*list(tresnet.children())[:-1])
#         self.SelectAdaptivePool2d = SelectAdaptivePool2d(flatten=True)

#         # self.model = tresnet.reset_classifier(num_classes)

#         self.age_model = nn.Sequential(
#             nn.Linear(tresnet.head.fc.in_features, 1024),
...
#             nn.Linear(tresnet.head.fc.in_features, 128),
#             nn.ReLU(), 
#             nn.Linear(128, 2),
#             nn.Sigmoid()
#         )
...
#         # flatten
#         # TODO: 比較.view與flatten結果差異
#         # x = x.view(x.size(0), -1)
#         x = self.SelectAdaptivePool2d(x) # torch.Size([32, 672])
...
#     def forward(self, x):
#         x = self.model(x) # torch.Size([32, 672, 7, 7])
