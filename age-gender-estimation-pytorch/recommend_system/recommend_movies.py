...
def recommend_movies(pretrained, inputs, linear_feature_columns, dnn_feature_columns, DEVICE, df):
    ...
    for i, ans_idx in enumerate(idx[:,0]):
        ...
        if i < 5:
            ...
            print(f"Predict rating: {pred_ans[ans_idx][0] :.3f}, movie_id: {data.iloc[ans_idx]['movie_id']}, movie_title: {data.iloc[ans_idx]['movie_title']}, gender: {data.iloc[ans_idx]['gender']}, age: {data.iloc[ans_idx]['age']}")
    ...
    return pred_movie_list, pred_rating
